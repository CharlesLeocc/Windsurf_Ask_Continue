...
StatusViewProvider.viewType = "askContinue.statusView";
/**
            ...
            case "continue":
                try {
                    ...
                    if (message.images && message.images.length > 0 && message.uploadType !== 'path') {
                        const imagesData = message.images.map((img, i) => '[图片 ' + (i + 1) + ': ' + img.name + ']\n' + img.base64).join('\n\n');
                        finalText = finalText + '\n\n' + imagesData;
                    }
                    // 处理非图片文件：附加文件路径信息
                    if (message.files && message.files.length > 0) {
                        const filesData = message.files.map((f, i) => '[文件 ' + (i + 1) + ': ' + f.name + ']' + (f.path ? '\n路径: ' + f.path : '')).join('\n\n');
                        finalText = finalText + '\n\n' + filesData;
                    }
