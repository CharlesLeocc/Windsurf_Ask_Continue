...
> 💡 **智能启动器**：安装后会在 `~/Documents/mcp/` 目录配置智能启动器，具备以下特性：
> - **Go 优先**：优先启动 Go 版本（连接更稳定）
> - **自动降级**：Go 失败时自动切换到 Python 版本
...
> ```
> ~/Documents/mcp/
> ├── ask-continue-mcp      # Go 版本可执行文件
> ├── mcp-launcher.sh       # 智能启动器脚本
> ├── mcp-launcher.log      # 运行日志
> └── mcp-server-python/    # Python 版本
>     ├── server.py
>     └── requirements.txt
> ```
